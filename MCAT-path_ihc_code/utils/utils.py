...
def get_optim(parameters, args):
    if args.opt == "adam":
        optimizer = optim.Adam(parameters, lr=args.lr, weight_decay=args.reg)
    elif args.opt == 'sgd':
        optimizer = optim.SGD(parameters, lr=args.lr, momentum=0.9, weight_decay=args.reg)
    else:
        raise NotImplementedError
    return optimizer
...
class Alignment_ContrastiveLoss(nn.Module):
   """
